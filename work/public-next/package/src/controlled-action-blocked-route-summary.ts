...
export type BlockedRouteStatus =
  | 'blocked_policy'
  | 'blocked_no_approval'
...
export function projectControlledActionBlockedRouteSummary(): ControlledActionBlockedRouteSummaryProjection {
  ...
  const routes = policy.routes.map((route) => {
    ...
    if (!route.routeVisibleAsMetadata) blockedReasons.add('blocked_policy');
    if (route.approvalRequired) blockedReasons.add('blocked_no_approval');
    if (route.allowlistRequired) blockedReasons.add('blocked_no_allowlist');
    if (!route.enabledNow) blockedReasons.add('blocked_write_actions_disabled');
    if (route.dryRunRequired) blockedReasons.add('blocked_dry_run_required');
    if (route.allowedExecutionMode === 'preview_only') blockedReasons.add('blocked_preview_required');
    if (route.backupRequired) blockedReasons.add('blocked_backup_required');
    if (route.backupRequired) blockedReasons.add('blocked_rollback_required');
    if (!policy.bridgePolicies.desktopBridge.enabledNow && route.scopeTarget.startsWith('desktop.')) blockedReasons.add('blocked_bridge_not_implemented');
    if (!policy.bridgePolicies.phaseBridge.enabledNow && route.scopeTarget.startsWith('phasebridge.')) blockedReasons.add('blocked_bridge_not_implemented');
    blockedReasons.add('blocked_direct_source_disabled');
    blockedReasons.add('blocked_automatic_takeover_disabled');
    if (!route.scopeKind || !route.scopeTarget) blockedReasons.add('blocked_scope_invalid');
    if (route.allowedExecutionMode === 'forbidden') blockedReasons.add('metadata_only');

    let routeStatus: BlockedRouteStatus = 'execution_ineligible';
    ...
    } satisfies ControlledActionBlockedRouteSummary;
  });
