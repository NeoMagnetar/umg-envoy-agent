...
function markerSymbol(marker: ControlledActionRuntimeReportPanelRow['marker']): string {
  switch (marker) {
    case 'ok':
      return '✓';
    case 'warning':
...
function panelStatusFromRows(rows: ControlledActionRuntimeReportPanelRow[]): ControlledActionRuntimeReportPanel['status'] {
  if (rows.some((row) => row.marker === 'blocked')) return 'blocked';
  if (rows.some((row) => row.marker === 'warning')) return 'warning';
  if (rows.some((row) => row.marker === 'ok')) return 'complete';
  return 'info';
}
...
function box(title: string, lines: string[], width = 70): string {
...
function buildEvidenceRows(input: ControlledActionRuntimeReportInput): ControlledActionRuntimeReportPanelRow[] {
  const complete = (present: boolean): ControlledActionRuntimeReportPanelRow['marker'] => (present ? 'ok' : 'warning');
  return [
...
export function buildControlledActionRuntimeReportNavigation(
  report: Pick<ControlledActionRuntimeReport, 'panels'>,
): ControlledActionRuntimeReportNavigationItem[] {
  return [
...
export function renderControlledActionRuntimeReportAscii(report: ControlledActionRuntimeReport): string {
...
export function projectControlledActionRuntimeReport(
  input: ControlledActionRuntimeReportInput,
  ...
  const activeRouteRows: ControlledActionRuntimeReportPanelRow[] = [
  ...
  const blockedCapabilities: ControlledActionRuntimeReportPanelRow[] = [
    'execute_action',
    'write_actions',
    'bridge_actions',
    'direct_source',
    ...
    'resleever_touch',
  ].map((label) => ({ label, value: 'blocked', marker: 'blocked' as const }));

  const readinessRows: ControlledActionRuntimeReportPanelRow[] = [
    ...
    ...input.blocked.blockedReasons.map((reason) => ({ label: 'blockedReason', value: reason, marker: 'blocked' as const })),
    ...(input.evidence.missingEvidenceItems > 0
      ? [{ label: 'missingEvidenceItems', value: String(input.evidence.missingEvidenceItems), marker: 'warning' as const }]
      : []),
  ];

  const auditRows: ControlledActionRuntimeReportPanelRow[] = [
  ...
  const recordingRows: ControlledActionRuntimeReportPanelRow[] = [
  ...
  const hardBoundaryRows: ControlledActionRuntimeReportPanelRow[] = [
  ...
  const nextRows: ControlledActionRuntimeReportPanelRow[] = [
  ...
  const panels: ControlledActionRuntimeReport['panels'] = {
  ...
  const overallStatus: ControlledActionRuntimeReport['overview']['overallStatus'] =
    route.routeStatus === 'incomplete'
      ? 'incomplete'
  ...
  const reportBase: Omit<ControlledActionRuntimeReport, 'navigation' | 'asciiReport'> = {
  ...
  const report: ControlledActionRuntimeReport = {
    ...reportBase,
