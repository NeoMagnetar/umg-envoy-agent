...
export function projectControlledActionAuditPacketReviewBundle(
  input: ControlledActionAuditPacketReviewBundleInput,
): ControlledActionAuditPacketReviewBundle {
  const attentionItems: ControlledActionAuditPacketReviewAttentionItem[] = [];
  ...
  const routeReviewCards = input.sourceExport.routes.map((route, index) => {
    let reviewStatus: ControlledActionAuditPacketRouteReviewCard['reviewStatus'] = 'review_attention_required';
    const reviewerFocus: string[] = [];
    ...
    return {
      ...
      approvalGranted: false,
      reviewDecisionRecorded: false,
    } satisfies ControlledActionAuditPacketRouteReviewCard;
  });
