...
export function projectControlledActionReviewDecisionPacketRecordingPhaseSummary(
  input: ControlledActionReviewDecisionPacketRecordingPhaseSummaryInput,
): ControlledActionReviewDecisionPacketRecordingPhaseSummary {
  const hard = input.hardBoundaries satisfies ControlledActionRecordingPhaseHardBoundaries;

  const boundarySummary = {
    recordingBoundaryIntact:
      hard.recordingImplemented === false &&
    ...
    transmissionBoundaryIntact:
      hard.fileWritten === false &&
  ...
  const boundariesIntact =
    boundarySummary.recordingBoundaryIntact &&
  ...
  let phaseStatus: ControlledActionReviewDecisionPacketRecordingPhaseSummary['phaseStatus'] = input.phaseStatus;
  if (!boundariesIntact) {
    phaseStatus = 'blocked';
  ...
  return {
    ...
    boundarySummary,
    recommendedNextLane:
      input.recommendedNextLane || 'ALPHA9_CONTROLLED_ACTION_EXECUTION_PHASE_HANDOFF_REPORT_SOURCE',
