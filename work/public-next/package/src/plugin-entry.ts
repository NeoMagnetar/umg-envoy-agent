  ...
  api.registerCli(({ program }: { program: any }) => {
    ...
    root.command("sleeve-select")
      ...
      .action(async (opts: { sleeveId?: string; selectionMode?: string; persistSelection?: boolean; runtimeSessionId?: string; root?: string }) => {
        console.log(JSON.stringify(selectRuntimeSleeve(UMG_ENVOY_RUNTIME_VERSION, "dist/plugin-entry.js", opts.root ?? defaultBlockLibraryRoot(), { sleeveId: opts.sleeveId, selectionMode: opts.selectionMode as any, persistSelection: Boolean(opts.persistSelection), runtimeSessionId: opts.runtimeSessionId }), null, 2));
      });
    ...
    root.command("runtime-compile")
      ...
      .action(async (opts: { sleeveId?: string; runtimeSessionId?: string; resolveDepth?: string; strictness?: string; compileMode?: string; root?: string }) => {
        console.log(JSON.stringify(compileRuntimeSleeve(UMG_ENVOY_RUNTIME_VERSION, "dist/plugin-entry.js", opts.root ?? defaultBlockLibraryRoot(), { sleeveId: opts.sleeveId, runtimeSessionId: opts.runtimeSessionId, resolveDepth: opts.resolveDepth as any, strictness: opts.strictness as any, compileMode: opts.compileMode ?? 'dry_run' }), null, 2));
      });
    ...
    root.command("runtime-report")
      ...
      .action(async (opts: { mode?: string; panel?: string; sleeveId?: string; routeId?: string; includeAscii?: boolean; includeNavigation?: boolean; includeStructuredReport?: boolean }) => {
        console.log(JSON.stringify(executeControlledActionRuntimeReportPluginOwnedAccess({ reportMode: opts.mode as any, panel: opts.panel as any, sleeveId: opts.sleeveId, routeId: opts.routeId, includeAscii: opts.includeAscii, includeNavigation: opts.includeNavigation, includeStructuredReport: opts.includeStructuredReport }), null, 2));
      });
  }, { commands: ["umg-envoy"] });
}
...
api.registerTool({ name: "umg_envoy_controlled_action_runtime_report", description: "Return a read-only controlled-action runtime report preview with structured report, ASCII dashboard, navigation, or selected panel output.", parameters: ControlledActionRuntimeReportToolInputSchema, async execute(input: any) { return { content: [{ type: "text", text: JSON.stringify(executeControlledActionRuntimeReportTool(input), null, 2) }] }; } }, { optional: true });
api.registerTool({ name: "umg_envoy_controlled_action_runtime_report_access", description: "Expose an Envoy-owned read-only access surface for controlled-action runtime report verification without execution, approval, recording, writes, or external transmission.", parameters: ControlledActionRuntimeReportPluginOwnedAccessInputSchema, async execute(input: any) { return { content: [{ type: "text", text: JSON.stringify(executeControlledActionRuntimeReportPluginOwnedAccess(input), null, 2) }] }; } }, { optional: true });
  }
};
