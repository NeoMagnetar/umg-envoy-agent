...
import { defaultBlockLibraryRoot, getBlockLibraryActiveStackProjection, getBlockLibraryManifestEntryLookup, getBlockLibraryManifestIndex, getBlockLibraryMoltMapCompose, getBlockLibraryMoltMapFragment, getBlockLibraryMoltblockVisibleExtract, getBlockLibraryNeoblockInspect, getBlockLibraryResponseEnvelopeFragment, getBlockLibraryStatus, getBlockLibraryTargetShallowLoadGate, getBlockLibraryTargetShallowLoadSingle, getBlockLibraryTargetShallowSummaryNormalize } from "./block-library-resolver.js";
...
function registerCliBridge(api: any, config?: PluginConfig) {
  api.registerCli(({ program }: { program: any }) => {
    ...
    root.command("block-library-response-envelope-fragment")
      ...
      .option("--include-raw")
      .option("--root <path>")
      .action(async (opts: { neoblockIds: string; project?: string; currentState?: string; activeTool?: string; formalResponseContent?: string; envoyIntuition?: string; projectionFormat?: string; includeMetadata?: boolean; includeAudit?: boolean; includeRaw?: boolean; root?: string }) => {
        console.log(JSON.stringify(getBlockLibraryResponseEnvelopeFragment("0.3.0-alpha.6", "dist/plugin-entry.js", opts.root ?? defaultBlockLibraryRoot(), { neoblockIds: String(opts.neoblockIds).split(',').map((v) => v.trim()).filter(Boolean), project: opts.project, currentState: opts.currentState, activeTool: opts.activeTool, formalResponseContent: opts.formalResponseContent, envoyIntuition: opts.envoyIntuition, projectionFormat: opts.projectionFormat ?? 'both', includeMetadata: opts.includeMetadata !== false, includeAudit: opts.includeAudit !== false, includeRaw: Boolean(opts.includeRaw) }), null, 2));
      });
