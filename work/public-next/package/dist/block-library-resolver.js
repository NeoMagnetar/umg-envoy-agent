...
function getRuntimePackageRootFromModuleUrl(moduleUrl) {
    if (!moduleUrl)
        return null;
    try {
...
function buildNativeGraphRichnessSummaries(nativeProjection, graphRunId, sessionState, sleeveId) {
    ...
    const neoStackSummary = {
        ...
        items: (nativeGraph?.neoStacks ?? []).map((stack) => ({
    ...
    const neoBlockSummary = {
        ...
        items: (nativeGraph?.neoBlocks ?? []).map((block) => ({
            ...
            visibleMoltFragmentCount: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.sourceNeoBlockId === block.neoBlockId).length,
            extractedMoltTypes: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.sourceNeoBlockId === block.neoBlockId).map((fragment) => fragment.moltType),
    ...
    const moltFragmentSummary = {
        ...
        groups: {
            Trigger: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.moltType === 'Trigger'),
            Directive: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.moltType === 'Directive'),
            Instruction: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.moltType === 'Instruction'),
            Subject: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.moltType === 'Subject'),
            Primary: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.moltType === 'Primary'),
            Philosophy: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.moltType === 'Philosophy'),
            Blueprint: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.moltType === 'Blueprint'),
            Off: [],
            excluded: []
        }
    };
    const toolRequestSummary = {
        ...
        requests: (nativeGraph?.toolRequests ?? []).map((request) => ({
    ...
    const irMatrixSummary = {
        ...
        activeRoute: nativeGraph?.irRoutes?.[0]?.nodes?.map((node) => node.nodeId) ?? [],
        blockedRoute: [],
        sourceMode: 'sleeve_native',
        sourceProvenance,
        routePurity: 'clean_native'
    ...
    return {
        ...
        envelopeSummary,
        diagnostics,
        graphCompleteness: 'rich_sleeve_native'
    };
}
...
export function inspectRuntimeSleeveGraphRichness(version, entrypoint = 'dist/plugin-entry.js', root = DEFAULT_LIBRARY_ROOT, input = {}) {
    ...
    if (!sleeveId) {
        return {
            ok: true,
            outputContract: { contractId: 'umg.runtime.sleeve_graph.richness.v1', contractStatus: 'NORMALIZED' },
    ...
    const neoBlockItems = useNativeProjection
        ? (nativeGraph?.neoBlocks ?? []).map((block) => ({
            neoBlockId: block.neoBlockId,
            parentNeoStackId: Array.isArray(block.parentStackIds) ? (block.parentStackIds[0] ?? null) : null,
            parentNeoStackIds: block.parentStackIds ?? [],
            declaredRef: block.neoBlockId,
    ...
    const moltGroups = useNativeProjection
        ? {
            Trigger: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.moltType === 'Trigger').map((fragment) => ({ sourceNeoBlockId: fragment.sourceNeoBlockId, sourceNeoStackId: null, moltType: fragment.moltType, contentSummary: fragment.content, activeStatus: 'active', exclusionReason: null, mergeKey: fragment.fragmentId, stackKey: null, trace: [`sourceNeoBlockId=${fragment.sourceNeoBlockId}`], sourceMode: 'sleeve_native', provenance: fragment.provenance ?? null })),
            Directive: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.moltType === 'Directive').map((fragment) => ({ sourceNeoBlockId: fragment.sourceNeoBlockId, sourceNeoStackId: null, moltType: fragment.moltType, contentSummary: fragment.content, activeStatus: 'active', exclusionReason: null, mergeKey: fragment.fragmentId, stackKey: null, trace: [`sourceNeoBlockId=${fragment.sourceNeoBlockId}`], sourceMode: 'sleeve_native', provenance: fragment.provenance ?? null })),
            Instruction: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.moltType === 'Instruction').map((fragment) => ({ sourceNeoBlockId: fragment.sourceNeoBlockId, sourceNeoStackId: null, moltType: fragment.moltType, contentSummary: fragment.content, activeStatus: 'active', exclusionReason: null, mergeKey: fragment.fragmentId, stackKey: null, trace: [`sourceNeoBlockId=${fragment.sourceNeoBlockId}`], sourceMode: 'sleeve_native', provenance: fragment.provenance ?? null })),
            Subject: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.moltType === 'Subject').map((fragment) => ({ sourceNeoBlockId: fragment.sourceNeoBlockId, sourceNeoStackId: null, moltType: fragment.moltType, contentSummary: fragment.content, activeStatus: 'active', exclusionReason: null, mergeKey: fragment.fragmentId, stackKey: null, trace: [`sourceNeoBlockId=${fragment.sourceNeoBlockId}`], sourceMode: 'sleeve_native', provenance: fragment.provenance ?? null })),
            Primary: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.moltType === 'Primary').map((fragment) => ({ sourceNeoBlockId: fragment.sourceNeoBlockId, sourceNeoStackId: null, moltType: fragment.moltType, contentSummary: fragment.content, activeStatus: 'active', exclusionReason: null, mergeKey: fragment.fragmentId, stackKey: null, trace: [`sourceNeoBlockId=${fragment.sourceNeoBlockId}`], sourceMode: 'sleeve_native', provenance: fragment.provenance ?? null })),
            Philosophy: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.moltType === 'Philosophy').map((fragment) => ({ sourceNeoBlockId: fragment.sourceNeoBlockId, sourceNeoStackId: null, moltType: fragment.moltType, contentSummary: fragment.content, activeStatus: 'active', exclusionReason: null, mergeKey: fragment.fragmentId, stackKey: null, trace: [`sourceNeoBlockId=${fragment.sourceNeoBlockId}`], sourceMode: 'sleeve_native', provenance: fragment.provenance ?? null })),
            Blueprint: (nativeGraph?.moltFragments ?? []).filter((fragment) => fragment.moltType === 'Blueprint').map((fragment) => ({ sourceNeoBlockId: fragment.sourceNeoBlockId, sourceNeoStackId: null, moltType: fragment.moltType, contentSummary: fragment.content, activeStatus: 'active', exclusionReason: null, mergeKey: fragment.fragmentId, stackKey: null, trace: [`sourceNeoBlockId=${fragment.sourceNeoBlockId}`], sourceMode: 'sleeve_native', provenance: fragment.provenance ?? null })),
            Off: [],
            excluded: []
        }
    ...
    const toolRequests = useNativeProjection
        ? (nativeGraph?.toolRequests ?? []).map((request) => ({
            requestId: request.requestId,
            requestedToolName: request.requestedToolName,
            requestedAction: request.requestedAction,
            classification: 'available_read_only',
        ...
        : (Array.isArray(classifier.classifications) ? classifier.classifications : []);
    const runtimeSpecValue = inspection.runtimeSpec;
    const runtimePreviewSpecValue = runtimePreview.runtimeSpec;
    ...
    const baseRichnessResult = {
        ok: true,
        outputContract: { contractId: 'umg.runtime.sleeve_graph.richness.v1', contractStatus: 'NORMALIZED' },
        graphStatus: inspection.ok ? (graphCompleteness === 'partial' || graphCompleteness === 'neoblock_only' ? 'GRAPH_PARTIAL' : 'GRAPH_READY') : 'GRAPH_ERROR',
        sourceSleeveId: sleeveId,
