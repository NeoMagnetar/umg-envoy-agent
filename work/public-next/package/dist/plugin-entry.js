...
function statusPayload(config) {
    ...
    return {
        ...
        version: UMG_ENVOY_RUNTIME_VERSION,
...
function compareSleeves(left, right, config) {
...
function registerCliBridge(api, config) {
    api.registerCli(({ program }) => {
        ...
        root.command("block-library-status")
            .option("--root <path>")
            .action(async (opts) => {
            console.log(JSON.stringify(getBlockLibraryStatus(UMG_ENVOY_RUNTIME_VERSION, "dist/plugin-entry.js", opts.root ?? defaultBlockLibraryRoot()), null, 2));
        });
        root.command("block-library-manifest-index")
            .option("--root <path>")
            .action(async (opts) => {
            console.log(JSON.stringify(getBlockLibraryManifestIndex(UMG_ENVOY_RUNTIME_VERSION, "dist/plugin-entry.js", opts.root ?? defaultBlockLibraryRoot()), null, 2));
        });
        ...
        root.command("sleeve-select")
            ...
            .option("--root <path>")
            .action(async (opts) => {
            console.log(JSON.stringify(selectRuntimeSleeve(UMG_ENVOY_RUNTIME_VERSION, "dist/plugin-entry.js", opts.root ?? defaultBlockLibraryRoot(), { sleeveId: opts.sleeveId, selectionMode: opts.selectionMode, persistSelection: Boolean(opts.persistSelection), runtimeSessionId: opts.runtimeSessionId }), null, 2));
        });
        root.command("sleeve-resolve")
            ...
            .option("--root <path>")
            .action(async (opts) => {
            console.log(JSON.stringify(resolveRuntimeSleeveGraph(UMG_ENVOY_RUNTIME_VERSION, "dist/plugin-entry.js", opts.root ?? defaultBlockLibraryRoot(), { sleeveId: opts.sleeveId, runtimeSessionId: opts.runtimeSessionId, resolveDepth: opts.resolveDepth, maxNeoStacks: opts.maxNeostacks ? Number(opts.maxNeostacks) : undefined, maxNeoBlocks: opts.maxNeoblocks ? Number(opts.maxNeoblocks) : undefined, maxVisibleMoltFragments: opts.maxVisibleMoltFragments ? Number(opts.maxVisibleMoltFragments) : undefined, allowRecursive: Boolean(opts.allowRecursive), mode: opts.mode ?? 'dry_run' }), null, 2));
        });
        root.command("runtime-compile")
            ...
            .option("--root <path>")
            .action(async (opts) => {
            console.log(JSON.stringify(compileRuntimeSleeve(UMG_ENVOY_RUNTIME_VERSION, "dist/plugin-entry.js", opts.root ?? defaultBlockLibraryRoot(), { sleeveId: opts.sleeveId, runtimeSessionId: opts.runtimeSessionId, resolveDepth: opts.resolveDepth, strictness: opts.strictness, compileMode: opts.compileMode ?? 'dry_run' }), null, 2));
        });
...
const entry = {
    ...
    register(api, config) {
        ...
        api.registerTool({ name: "umg_envoy_compile_sleeve", description: "Compile a bundled public sleeve into RuntimeSpec-like output.", parameters: Type.Object({ sleeveId: Type.String() }, { additionalProperties: false }), async execute(input) { return { content: [{ type: "text", text: JSON.stringify(compileSleeveById(input.sleeveId, config, import.meta.url), null, 2) }] }; } }, { optional: true });
        ...
        api.registerTool({ name: "umg_envoy_validate_path", description: "Validate a parsed UMG path string.", parameters: Type.Object({ source: Type.String() }, { additionalProperties: false }), async execute(input) { const issues = validateUMGPath(parseUMGPath(input.source)); return { content: [{ type: "text", text: JSON.stringify({ ok: issues.every((issue) => issue.severity !== "error"), issues }, null, 2) }] }; } }, { optional: true });
