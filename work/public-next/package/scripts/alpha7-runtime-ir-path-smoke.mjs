...
const tool = defs.find((def) => def.name === "umg_envoy_runtime_ir_path");
...
record("default call returns runtime IR path", async () => {
  ...
  assert(parsed.path.some((node) => node.kind === "sleeve" && node.id === "neomagnetar-dynamic-persona-v1"), "expected sleeve node in path");
  assert(parsed.path.some((node) => node.kind === "neostack" && node.label === "NO_DECLARED_NEOSTACKS"), "expected NO_DECLARED_NEOSTACKS marker");
  assert(parsed.path.some((node) => node.kind === "neoblock" && node.id === "primary.sample"), "expected primary.sample neoblock node");
  assert(parsed.path.some((node) => node.kind === "moltblock" && node.id === "primary.sample" && node.label === "Primary"), "expected primary.sample / Primary moltblock node");
  assert(parsed.edges.some((edge) => edge.relation === "references"), "expected references edge");
  assert(parsed.edges.some((edge) => edge.relation === "exposes_molt_summary"), "expected exposes_molt_summary edge");
  assert(parsed.summary.dormantRefCount === 6, `expected dormantRefCount=6, got ${parsed.summary.dormantRefCount}`);
  assert(parsed.dormantRefs.some((ref) => ref.id === "directive.sample"), "expected directive.sample dormant ref");
  assert(parsed.dormantRefs.some((ref) => ref.id === "trigger.sample"), "expected trigger.sample dormant ref");
  const lanes = new Map(parsed.excludedLanes.map((lane) => [lane.lane, lane]));
  ...
  assert(parsed.nlProjection.includes("Runtime IR Path"), "expected nlProjection Runtime IR Path");
  assert(parsed.nlProjection.includes("Dormant NeoBlock Refs"), "expected nlProjection Dormant NeoBlock Refs");
