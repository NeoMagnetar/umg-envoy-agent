import {
  canProceedLowRiskDirect,
  createBlockedUnknownToolGate,
  createLowRiskAllowlistPolicy,
...
const nonAllowlistedDecision = evaluateLowRiskDirectEligibility(
  lowRiskGate,
  capability({ toolId: "not-allowlisted.tool", toolName: "Not Allowlisted Tool", allowedRiskClass: "low_risk_direct", allowlistTags: [] }),
  policy,
);
...
const approvalDecision = evaluateLowRiskDirectEligibility(
  approvalGate,
  capability({
    toolId: "write.tool",
    toolName: "Write Tool",
...
const destructiveDecision = evaluateLowRiskDirectEligibility(
  destructiveGate,
  capability({
    toolId: "delete.tool",
...
const transmissionDecision = evaluateLowRiskDirectEligibility(
  transmissionGate,
  capability({
...
const previewDecision = evaluateLowRiskDirectEligibility(
  previewGate,
  capability({
    toolId: "preview.tool",
...
const dryRunDecision = evaluateLowRiskDirectEligibility(
  dryRunGate,
  capability({
    toolId: "dryrun.tool",
...
const invalidBoundaryDecision = evaluateLowRiskDirectEligibility(
  invalidBoundaryGate,
  capability({ toolId: "unsafe.tool", toolName: "Unsafe Tool", allowedRiskClass: "read_only" }),
  policy,
);
...
const invalidTraceDecision = evaluateLowRiskDirectEligibility(
  invalidTraceGate,
  capability({ toolId: "unsafe-trace.tool", toolName: "Unsafe Trace Tool", allowedRiskClass: "read_only" }),
  policy,
);
