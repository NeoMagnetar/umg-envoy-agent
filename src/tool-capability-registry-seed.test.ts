import {
  canToolRunDirectly,
  requiresDryRunForCapability,
  type ToolResult,
} from "./action-gate-types.js";
import {
  ENVOY_TOOL_CAPABILITY_REGISTRY_SEED,
  createEnvoyToolCapabilityRegistrySeed,
  resolveEnvoySeededToolCapability,
} from "./tool-capability-registry-seed.js";
...
assert("all 21 seeded tool ids are represented in the seed", ENVOY_TOOL_CAPABILITY_REGISTRY_SEED.length === 21 && expectedToolIds.every((toolId) => Boolean(resolveEnvoySeededToolCapability(toolId))));
...
assert("compose sleeve dry run is dry-run only", resolveEnvoySeededToolCapability("umg_envoy_compose_sleeve_dry_run")?.allowedRiskClass === "dry_run_only" && requiresDryRunForCapability(resolveEnvoySeededToolCapability("umg_envoy_compose_sleeve_dry_run") ?? null) === true);
assert("validate composed sleeve dry run is dry-run only", resolveEnvoySeededToolCapability("umg_envoy_validate_composed_sleeve_dry_run")?.allowedRiskClass === "dry_run_only" && requiresDryRunForCapability(resolveEnvoySeededToolCapability("umg_envoy_validate_composed_sleeve_dry_run") ?? null) === true);
