...
export type LowRiskDirectExecutionStatus =
  | "executed_success"
  | "executed_failure"
  | "execution_blocked"
  | "execution_denied";
...
type LowRiskHandler = (input: Record<string, unknown> | undefined) => unknown;
...
const LOW_RISK_DIRECT_HANDLER_MAP: Record<LowRiskDirectToolId, LowRiskHandler> = {
...
function createActionGateForRequest(request: LowRiskDirectExecutionRequest): ActionGate {
...
export function executeLowRiskDirectTool(request: LowRiskDirectExecutionRequest): LowRiskDirectExecutionResponse {
  const capability = resolveEnvoySeededToolCapability(request.toolId);
  const actionGate = createActionGateForRequest(request);

  const blockedDraft = (status: "execution_blocked" | "execution_denied", outputSummary: string, warnings: string[] = [], errors: string[] = []): LowRiskDirectExecutionResponse => ({
    ok: false,
    status,
    blocked: true,
    output: null,
  ...
  if (!LOW_RISK_DIRECT_TOOL_IDS.includes(request.toolId as LowRiskDirectToolId)) {
  ...
  const handler = LOW_RISK_DIRECT_HANDLER_MAP[request.toolId as LowRiskDirectToolId];
  if (!handler) {
