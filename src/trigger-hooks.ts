...
export type ResolvedTriggerBehavior =
  | ({ triggerId: string; priority: number } & TriggerBlockBehavior)
  | null;
...
function validateTriggerId(id: string): string | null {
  ...
  if (!id.startsWith(TRIGGER_ID_PREFIX)) return `id must start with ${TRIGGER_ID_PREFIX}`;
  if (!TRIGGER_ID_PATTERN.test(id)) return "id must use lowercase dotted format with letters, digits, underscore, and dot only";
  return null;
}

function parseTriggerBlock(filePath: string): { block: TriggerBlock | null; invalidReason?: string } {
  try {
    const raw = fs.readFileSync(filePath, "utf8");
    const parsed = JSON.parse(raw) as unknown;
    if (!isRecord(parsed)) return { block: null, invalidReason: "manifest is not an object" };
    ...
    let behavior: TriggerBlockBehavior | null = null;
...
export function loadTriggerCatalog(paths: ResolvedPaths): TriggerCatalog {
  const triggerRoots = [
    path.join(paths.resleeverBlocksDir, "triggers"),
    path.join(paths.resleeverBlocksDir, "manifests", "triggers")
  ];

  const parsedBlocks: TriggerBlock[] = [];
  const invalidManifests: TriggerDiagnostics["invalidManifests"] = [];
...
export function loadTriggerBlocks(paths: ResolvedPaths): TriggerBlock[] {
  return loadTriggerCatalog(paths).blocks;
}

function matchesPhrase(text: string, phrase: string, mode: TriggerBlockMatchMode): boolean {
...
export function resolveTriggerBehaviorFromUserText(text: string, triggerBlocks: TriggerBlock[]): ResolvedTriggerBehavior {
...
export function extractLatestUserText(messages: Array<{ role?: unknown; content?: unknown }>): string {
