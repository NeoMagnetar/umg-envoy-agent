...
import type {
  ...
  LoadedNeostackFile,
  NeostackValidationResult,
  NeostackArtifactResolutionResult
} from "../types.js";
...
function addEntry(entries: ArtifactResolutionEntry[], entry: ArtifactResolutionEntry) {
  ...
  if (entries.some((existing) => `${existing.artifactType}::${existing.artifactId}::${existing.sourceReason ?? ""}` === key)) return;
  entries.push(entry);
}

function resolveNeoblockPath(libraryRoot: string, artifactId: string): string {
  return path.join(path.resolve(libraryRoot), "AI", "NEOBLOCKS", "categories", "core", `${artifactId}.json`);
}

export function validateNeostackStructure(neostackFile: LoadedNeostackFile): NeostackValidationResult {
  const errors: string[] = [];
...
export function resolveNeostackArtifacts(libraryRoot: string, neostackFile: LoadedNeostackFile): NeostackArtifactResolutionResult {
  ...
  if (missing.length > 0) warnings.push(`${missing.length} referenced neoblocks were not found under the provided library root`);
