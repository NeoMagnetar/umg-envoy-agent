...
function cloneRegistry(registry: CognitiveRegistry): CognitiveRegistry {
  return JSON.parse(JSON.stringify(registry)) as CognitiveRegistry;
}

function cloneComposition(composition: SleeveComposerDryRunResult): SleeveComposerDryRunResult {
  return JSON.parse(JSON.stringify(composition)) as SleeveComposerDryRunResult;
}
