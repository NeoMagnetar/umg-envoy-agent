...
function edgeExists(result: ReturnType<typeof explainSleeveById>, input: { from?: string; to?: string; type: string }) {
  return result.matrix_summary.edges?.some((edge) =>
    edge.type === input.type &&
    (input.from === undefined || edge.from === input.from) &&
    (input.to === undefined || edge.to === input.to)
  ) === true;
}

function nodeExists(result: ReturnType<typeof explainSleeveById>, input: { id?: string; type?: string; name?: string }) {
  return result.matrix_summary.nodes?.some((node) =>
    (input.id === undefined || node.id === input.id) &&
    (input.type === undefined || node.type === input.type) &&
    (input.name === undefined || node.name === input.name)
  ) === true;
}
