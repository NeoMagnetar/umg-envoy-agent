...
import type { RelationMatrixConfig, RelationMatrixLine, RelationMatrixRequest, RelationMatrixResult } from "./relation-matrix-types.js";

function tempRootFromRequest(request: RelationMatrixRequest, mode: string): string {
...
export async function emitRelationMatrix(request: RelationMatrixRequest, config?: RelationMatrixConfig): Promise<RelationMatrixResult> {
  ...
  const runtimeSpec = bridgeResult.runtimeSpec as Record<string, unknown> | undefined;
  const trace = bridgeResult.trace as Record<string, unknown> | undefined;
  const diagnostics = bridgeResult.diagnostics as Record<string, unknown> | undefined;
  ...
  if (!relationMatrixEmitted || !runtimeSpec || !trace || !diagnostics) {
    return {
      ...
      bridgeResult: bridgeResult as unknown as Record<string, unknown>,
      runtimeSpec,
      trace,
      diagnostics,
  ...
  const lines = buildLines(bridgeResult as unknown as Record<string, any>);
  ...
  let relationMatrixPath: string | null = null;
  ...
  return {
    ...
    bridgeResult: bridgeResult as unknown as Record<string, unknown>,
    runtimeSpec,
    runtimeSpecBoundary: bridgeResult.runtimeSpecBoundary,
    trace,
    traceBoundary: bridgeResult.traceBoundary,
    diagnostics,
    relationMatrixText,
    relationMatrix,
    relationMatrixPath,
