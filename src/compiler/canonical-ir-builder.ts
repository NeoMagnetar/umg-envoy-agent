...
function artifactRefFromResolution(type: string, artifactId: string) {
  return `${type}:${artifactId}`;
}

function dependencyCounts(loadedSleeve: LoadedSleeveFile) {
...
function buildNodes(sleeve: LoadedSleeveFile, resolution: ArtifactResolutionResult) {
...
function buildEdges(sleeve: LoadedSleeveFile, resolution: ArtifactResolutionResult) {
  const artifactId = sleeve.identity?.artifact_id ?? "UNKNOWN.SLEEVE";
...
function buildRoutes(sleeve: LoadedSleeveFile) {
...
export function buildCanonicalIr(loadResult: SleeveLoadResult, libraryRoot: string, tempInputPath: string): CanonicalIrBuildResult {
  ...
  const preview: CompilerInputPreview = {
