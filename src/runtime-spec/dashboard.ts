...
export function renderRuntimeDashboard(dashboard: RuntimeDashboardV0): string {
  ...
  if (dashboard.governed_alpha) {
    ...
    if (dashboard.governed_alpha.payload && typeof dashboard.governed_alpha.payload === 'object') {
      const payload = dashboard.governed_alpha.payload as Record<string, unknown>;
      if (typeof payload.limit === 'number') lines.push(`Limit: ${payload.limit}`);
      if (typeof payload.hard_max === 'number') lines.push(`Hard Max: ${payload.hard_max}`);
    }
