...
export function renderUMGRuntimeDisplay(display: UMGRuntimeDisplayContractV0): string {
  const sections = [
    [
      "```nl",
      "Runtime Header:",
...
function collectWarnings(dashboard: RuntimeDashboardV0): string[] {
  const warnings = new Set<string>();
  for (const warning of dashboard.governed_handoff?.warnings ?? []) warnings.add(warning);
  for (const warning of dashboard.preflight?.warnings ?? []) warnings.add(warning);
  for (const warning of dashboard.governed_alpha?.warnings ?? []) warnings.add(warning);
  for (const warning of dashboard.local_readonly_inspection?.warnings ?? []) warnings.add(warning);
  return [...warnings];
}
