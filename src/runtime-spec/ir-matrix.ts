...
import type { RuntimeIRMatrixEdge, RuntimeIRMatrixNode, RuntimeIRMatrixNodeKind, RuntimeIRMatrixNodeState, RuntimeIRMatrixV0 } from "./ir-matrix-types.js";
...
export function buildRuntimeIRMatrix(input: {
  ...
  const nodes: RuntimeIRMatrixNode[] = [];
  const edges: RuntimeIRMatrixEdge[] = [];
  const warningSet = new Set<string>(spec.trace.warnings);

  const pushNode = (node: RuntimeIRMatrixNode) => {
    if (!nodes.some((existing) => existing.id === node.id)) {
      nodes.push(node);
    }
  };

  const pushEdge = (edge: RuntimeIRMatrixEdge) => {
  ...
  const matrix: RuntimeIRMatrixV0 = {
...
export function renderRuntimeIRMatrix(matrix: RuntimeIRMatrixV0): string {
  const lines: string[] = [];
  ...
  for (const node of matrix.nodes) {
    if (node.id === root.id || emitted.has(node.id)) continue;
...
function renderEdgeLine(node: RuntimeIRMatrixNode, edge: RuntimeIRMatrixEdge, depth = 0): string {
...
function symbolForKind(kind: RuntimeIRMatrixNodeKind): string {
  switch (kind) {
    case "runtime_spec":
    case "sleeve":
      return "◆";
...
function symbolForState(state: RuntimeIRMatrixNodeState): string {
  switch (state) {
    case "active":
    case "selected":
      return "●";
...
function provenanceForArtifact(spec: RuntimeSpecV0, artifactId: string): Record<string, unknown> {
...
function defaultTargetsForField(fieldName: string, rootId: string, governanceId: string, spec: RuntimeSpecV0): string[] {
  switch (fieldName) {
    case "Directive":
