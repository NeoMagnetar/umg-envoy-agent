...
import type { ActiveSleeveSelectionV0, SleeveSelectionCandidateV0, SleeveSelectionConfidence } from "./sleeve-selection-types.js";
...
function scoreSleeveCandidate(artifact: NormalizedArtifact, input: {
  user_task: string;
  selected_neostacks: string[];
  selected_neoblocks: string[];
  selected_molt_blocks: string[];
...
function isEligibleSleeve(artifact: NormalizedArtifact, warnings: string[]): boolean {
  const raw = artifact.raw as Record<string, unknown> | undefined;
  const statusValue = normalizeSleeveStatus(artifact, raw);
  if (artifact.kind !== "sleeve") return false;
  if (!artifact.source.canonical) {
    warnings.push("non-canonical source rejected");
    return false;
  }
...
function normalizeSleeveStatus(artifact: NormalizedArtifact, raw?: Record<string, unknown>): string {
  const sleeveStatus = typeof raw?.sleeve === "object" && raw.sleeve && typeof (raw.sleeve as Record<string, unknown>).status === "string"
    ? String((raw.sleeve as Record<string, unknown>).status)
    : undefined;
...
function getStringArray(raw: Record<string, unknown> | undefined, paths: string[]): string[] {
  const values: string[] = [];
...
function flattenStrings(value: unknown): string[] {
  ...
  if (typeof value === "object") return Object.values(value as Record<string, unknown>).flatMap((entry) => flattenStrings(entry));
  return [];
}

function provenanceForArtifact(artifact: NormalizedArtifact): SleeveSelectionCandidateV0["provenance"] {
  const raw = artifact.raw as Record<string, unknown> | undefined;
  return {
...
function requiresUnsafeExecution(raw: Record<string, unknown> | undefined, governance: RuntimeSpecGovernance): boolean {
  const runtimeServices = getStringArray(raw, ["sleeve.runtime.services", "runtime.services"]);
  if (governance.execution_mode !== "dry_run") return true;
  return runtimeServices.some((service) => /execute|write|live/i.test(service));
}

function requiresGovernanceExpansion(raw: Record<string, unknown> | undefined, governance: RuntimeSpecGovernance): boolean {
  ...
  if (values.some((value) => /mcp/i.test(value)) && governance.mcp_policy !== "metadata_only") return true;
  if (values.some((value) => /langchain/i.test(value)) && !["dry_run", "governed"].includes(governance.langchain_policy)) return true;
  return false;
}

function confidenceForScore(score: number, warnings: string[]): SleeveSelectionConfidence {
  if (score <= 0) return "none";
  if (warnings.some(isHardBlockingWarning)) return "none";
  if (score >= 75) return "high";
  if (score >= 45) return "medium";
  return "low";
}

function isHardBlockingWarning(warning: string): boolean {
  return [
...
function strongestConfidence(candidates: SleeveSelectionCandidateV0[]): SleeveSelectionConfidence {
  if (candidates.some((candidate) => candidate.confidence === "high")) return "high";
...
function inferNoMatchWarnings(input: { selected_neostacks: string[]; user_task: string }): string[] {
