...
import type { RuntimeInspectionArtifactSummaryV0, RuntimeInspectionRelationV0, RuntimeInspectionRequestV0, RuntimeInspectionResultV0 } from "./drilldown-types.js";
...
export function inspectRuntimeDrilldown(input: {
  ...
  const request = { depth: 1 as const, ...input.request };
  ...
  const result: RuntimeInspectionResultV0 = {
  ...
  if (artifact && shouldIncludeChildren(request.query_type)) {
    ...
    if (children.warnings.length > 0) warnings.push(...children.warnings);
  }
  ...
  if (request.include_support_docs || request.query_type === "inspect_support_docs") {
    ...
    if (supportDocs.warnings.length > 0) warnings.push(...supportDocs.warnings);
  }
  ...
  if (request.include_matrix_links || request.query_type === "inspect_matrix_links") {
    ...
    if (matrixLinks.warnings.length > 0) warnings.push(...matrixLinks.warnings);
  }
  ...
  if (request.include_molt_map_links || request.query_type === "inspect_molt_map_links") {
    ...
    if (moltLinks.warnings.length > 0) warnings.push(...moltLinks.warnings);
  }

  return result;
}

function selectSource(request: RuntimeInspectionRequestV0, runtimeSpec?: RuntimeSpecV0, dashboard?: RuntimeDashboardV0): RuntimeInspectionResultV0["source"] {
  if (request.query_type === "runtime_selection" && runtimeSpec) return "RuntimeSpecV0";
...
function createInspectionId(request: RuntimeInspectionRequestV0): string {
...
function resolveArtifact(request: RuntimeInspectionRequestV0, registryArtifacts: NormalizedArtifact[], runtimeSpec?: RuntimeSpecV0): NormalizedArtifact | undefined {
...
function toArtifactSummary(artifact: NormalizedArtifact): RuntimeInspectionArtifactSummaryV0 {
  return {
    id: artifact.id,
    kind: artifact.kind,
...
function shouldIncludeChildren(queryType: RuntimeInspectionRequestV0["query_type"]): boolean {
  return ["inspect_artifact", "inspect_sleeve", "inspect_neostack", "inspect_neoblock", "inspect_molt_block"].includes(queryType);
}

function resolveDeclaredChildren(artifact: NormalizedArtifact, registryArtifacts: NormalizedArtifact[], depth: number) {
  const childIds = new Set<string>();
  const relations: RuntimeInspectionRelationV0[] = [];
  const warnings: string[] = [];
  const children: RuntimeInspectionArtifactSummaryV0[] = [];
  if (depth <= 0) {
  ...
  for (const childId of childIds) {
    ...
    if (!child) continue;
    if (child.support_only) continue;
...
function resolveToolBindingRelations(request: RuntimeInspectionRequestV0, artifact: NormalizedArtifact | undefined, runtimeSpec: RuntimeSpecV0): RuntimeInspectionRelationV0[] {
  const relations: RuntimeInspectionRelationV0[] = [];
  ...
  if (!selectedArtifact) return relations;
...
function resolveSupportDocs(request: RuntimeInspectionRequestV0, artifact: NormalizedArtifact | undefined, runtimeSpec: RuntimeSpecV0 | undefined, supportArtifacts: NormalizedArtifact[]) {
  const support_docs: RuntimeInspectionArtifactSummaryV0[] = [];
  const relations: RuntimeInspectionRelationV0[] = [];
  const warnings: string[] = [];
  const artifactId = artifact?.id;
...
function resolveMatrixLinks(request: RuntimeInspectionRequestV0, artifact: NormalizedArtifact | undefined, irMatrix?: RuntimeIRMatrixV0) {
  const relations: RuntimeInspectionRelationV0[] = [];
  const warnings: string[] = [];
...
function mapMatrixRelation(relation: string): RuntimeInspectionRelationV0["relation"] {
  if (relation === "maps_to_molt_field") return "maps_to_molt_field";
  if (relation === "requests_tool") return "requests_tool";
  if (relation === "requires_approval") return "requires_approval";
  return "references";
}

function resolveMoltMapLinks(request: RuntimeInspectionRequestV0, artifact: NormalizedArtifact | undefined, moltMap?: RuntimeMOLTMapV0) {
  const relations: RuntimeInspectionRelationV0[] = [];
  const warnings: string[] = [];
  if (!artifact || !moltMap) {
...
function asStringArray(value: unknown): string[] {
  ...
  if (typeof value === "object") {
    return Object.values(value as Record<string, unknown>).flatMap((entry) => asStringArray(entry));
  }
