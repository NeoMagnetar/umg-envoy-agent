...
import type { GovernedApprovalItemV0, GovernedBlockedItemV0, GovernedExecutionHandoffStatus, GovernedExecutionHandoffV0 } from "./governed-execution-handoff-types.js";
...
export function buildGovernedExecutionHandoffDryRun(input: {
  ...
  const approvalItems = bindings
    ...
    .map<GovernedApprovalItemV0>((binding) => ({
  ...
  const blockedItems = bindings
    ...
    .map<GovernedBlockedItemV0>((binding) => ({
      tool_id: binding.tool_id,
      risk_level: binding.risk_level,
...
function resolveHandoffStatus(runtimeSpec: RuntimeSpecV0, bindings: RuntimeSpecV0["tool_bindings"]["bindings"], approvalItems: GovernedApprovalItemV0[], blockedItems: GovernedBlockedItemV0[]): GovernedExecutionHandoffStatus {
  if ((runtimeSpec.tool_bindings.requested.length === 0) && (!bindings || bindings.length === 0)) return "not_requested";
  if (blockedItems.length > 0) return "blocked";
  if (approvalItems.length > 0) return "requires_approval";
  if ((runtimeSpec.tool_bindings.metadata_only ?? []).length > 0 && runtimeSpec.tool_bindings.available.length === 0 && runtimeSpec.tool_bindings.requires_approval.length === 0) return "metadata_only";
  if ((runtimeSpec.tool_bindings.mock_only ?? []).length > 0 && runtimeSpec.tool_bindings.available.length === 0 && runtimeSpec.tool_bindings.requires_approval.length === 0) return "mock_only";
  if (runtimeSpec.tool_bindings.available.length > 0) return "draft";
  return "blocked";
}

function resolveCheckpointPolicy(status: GovernedExecutionHandoffStatus): GovernedExecutionHandoffV0["checkpoint"] {
  switch (status) {
...
function approvalReason(toolId: string): string {
  if (toolId === "langchain.agent_mode") return "LangChain agent mode requires approval.";
  if (toolId.includes("file_write")) return "File write requires approval.";
  if (toolId.includes("workflow_execute")) return "Workflow execution requires approval.";
  return "Tool requires approval under governed execution policy.";
}

function unknownBlockedReason(toolId: string): string {
  if (toolId.includes("remote")) return "remote execution blocked by policy";
  return "unknown tool cannot be handed to execution safely";
}

function handoffId(runtimeSpecId: string, traceId: string, status: GovernedExecutionHandoffStatus): string {
