...
import { createActionGateRuntimeReport, createActionGateRuntimeReportToolResponse, createProposedActionGate, type ActionGateRuntimeReportViewMode } from "./action-gate-types.js";
...
function registerCliBridge(api: any, config?: PluginConfig) {
  api.registerCli(({ program }: { program: any }) => {
    ...
    root.command("explain-sleeve")
      ...
      .action(async (opts: { sleeve: string; includeRuntimeSpec?: boolean; includeMatrixSummary?: boolean; runtimeSpec?: boolean }) => {
...
const entry = {
  ...
  register(api: { registerTool: (definition: any, options?: { optional?: boolean }) => void; registerCli?: (register: any, options?: { commands?: string[] }) => void }, config?: PluginConfig) {
    ...
    api.registerTool({ name: "umg_envoy_compile_sleeve", description: "Compile a bundled public sleeve into a non-executing RuntimeSpec artifact.", parameters: Type.Object({ sleeveId: Type.String() }, { additionalProperties: false }), async execute(input: { sleeveId: string }) { return { content: [{ type: "text", text: JSON.stringify(compileSleeveById(input.sleeveId, config, import.meta.url), null, 2) }] }; } }, { optional: true });
    api.registerTool({
      ...
      async execute(input: { sleeveId: string; includeRuntimeSpec?: boolean; includeMatrixSummary?: boolean }) {
    ...
    api.registerTool({
      ...
      async execute(input: { toolId: string; toolName?: string; mode?: ActionGateRuntimeReportViewMode }) {
    ...
    api.registerTool({
      ...
      async execute(input: { toolId: string; input?: Record<string, unknown> }) {
