# Release Notes — UMG Envoy Agent 0.3.0-alpha.14 — bounded low-risk direct runtime runner

## Summary

UMG Envoy Agent 0.3.0-alpha.14 continues the public alpha-track release line with a bounded runtime-facing capability story built around RuntimeSpecBoundary, TraceBoundary, ActionGate-controlled execution eligibility, ToolCapabilityRegistry seed policy, ToolResult audit records, runtime report inspection, and a tightly scoped low-risk direct runtime runner.
...
The current direct runner is limited to exactly six approved static safe read-only tools:
- `umg_envoy_status`
