...
function validateAndNormalizeConfig(input, sourceLabel) {
    const trace = [];
    const errors = [];
    const record = input;
    const bridge = (record?.mcp_bridge ?? input);
...
export function loadMcpBridgeConfig(explicitPath) {
    const trace = [
