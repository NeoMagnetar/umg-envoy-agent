...
export function validateSleeveStructure(input) {
    const errors = [];
    const warnings = [];
    if (!isRecord(input)) {
    ...
    if (!isRecord(artifact.sleeve)) {
        ...
        if (!isRecord(dependencies)) {
            ...
            for (const key of ["sleeve_ids", "neostack_ids", "bundle_ids", "overlay_ids", "schema_ids"]) {
        ...
        if (!isRecord(composition)) {
            ...
            for (const key of ["neostack_ids", "bundle_ids", "overlay_ids", "local_refs"]) {
