...
function resolveList(entries, libraryRoot, artifactType, values, sourceReason, sourceRef) {
...
function resolveNeoblocksFromLocalRefs(entries, libraryRoot, values) {
    if (!Array.isArray(values)) {
        return;
    }
...
function resolveNeoblocksFromNeostacks(entries, libraryRoot) {
    ...
    for (const stack of stacks) {
        try {
            ...
            const blockIds = [
                ...
                ...(Array.isArray(stackSection?.blocks) ? stackSection.blocks.map((item) => typeof item === "string" ? item : item?.artifact_id ?? item?.block_id).filter(Boolean) : [])
            ];
