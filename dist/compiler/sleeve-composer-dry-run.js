...
function resolveNeoBlocks(registry, stack) {
...
function resolveMoltBlocks(registry, neoBlocks) {
...
export function composeSleeveDryRun(input) {
    ...
    const resolvedNeoBlocks = neoBlocks.resolved.map(({ ref, neoblock }) => ({
        id: neoblock.id,
        name: neoblock.name,
    ...
    const steps = resolvedNeoBlocks.map((neoblock) => ({
