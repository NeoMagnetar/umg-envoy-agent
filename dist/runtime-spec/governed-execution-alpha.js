...
function resolverLibrarySearchPayload(input) {
...
function summaryRow(tool_id, status, risk_level, execution_mode, approval_required, governance_policy, alpha_eligible, blocked_reason) {
...
function resolveBinding(tool_id, handoff) {
...
function blockReason(toolId) {
    if (toolId === "mcp.server_metadata")
        return "candidate-only / not implemented in metadata alpha v2";
    if (toolId === "desktop_bridge.file_scan")
        return "local inspection outside metadata alpha v2";
    if (IMPLEMENTATION_BLOCKLIST.has(toolId))
        return "not in metadata alpha implementation allowlist";
    return "tool is outside the governed metadata alpha allowlist";
}
function inferBlockedRisk(toolId) {
    if (toolId.includes("delete"))
        return "destructive";
    if (toolId.includes("write") || toolId.includes("publish"))
        return "high";
    if (toolId.includes("langchain") || toolId.includes("mcp"))
        return "high";
    return "medium";
}
