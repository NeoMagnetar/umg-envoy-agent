export const DEFAULT_KNOWN_TOOL_SURFACES = [
    {
...
function resolveSelectedArtifacts(runtimeSpec, registryArtifacts) {
    const ids = [
...
function extractArtifactToolIds(artifact) {
...
function buildBindingFromArtifact(toolId, artifact, knownTools) {
...
function buildBindingFromRuntimeSpec(toolId, runtimeSpec, knownTools, extraWarnings = []) {
    const known = resolveKnownTool(toolId, knownTools);
    return {
        tool_id: known?.tool_id ?? toolId,
        label: known?.tool_id ?? toolId,
        ...
        status: statusFromKnownTool(known),
        risk_level: known?.default_risk_level ?? "unknown",
        execution_mode: known?.default_execution_mode ?? "dry_run",
        approval_required: Boolean(known?.approval_required),
        blocked_reason: known?.status === "blocked" ? known.governance_policy ?? "blocked by governance" : undefined,
        governance_policy: known?.governance_policy,
...
function finalizeBinding(binding, runtimeSpec, knownTools) {
    ...
    if (runtimeSpec.tool_bindings.blocked.includes(binding.tool_id) || known?.status === "blocked") {
        ...
        if (!blockedReason)
            blockedReason = known?.governance_policy ?? "blocked by governance";
    }
...
function normalizeArtifactKind(kind) {
    if (kind === "sleeve" || kind === "neostack" || kind === "neoblock" || kind === "molt_block")
        return kind;
    return "runtime_spec";
}
function resolveKnownTool(toolId, knownTools) {
...
function statusFromKnownTool(known) {
    switch (known?.status) {
        case "metadata_only":
...
function getFieldStrings(raw, fieldPath) {
    if (!raw)
        return [];
...
function flattenStrings(value) {
    ...
    if (typeof value === "object")
        return Object.entries(value).flatMap(([key, entry]) => [key, ...flattenStrings(entry)]);
    return [];
}
function mapLooseToolReference(value) {
    const lower = value.toLowerCase();
    const hits = [];
    if (lower.includes("langchain.agent_mode") || lower.includes("agent mode"))
        hits.push("langchain.agent_mode");
    if (lower.includes("langchain_bridge") || lower.includes("langchain bridge") || lower === "langchain")
        hits.push("langchain_bridge");
    if (lower.includes("resolver.library_status") || lower.includes("library status") || lower.includes("resolver status"))
        hits.push("resolver.library_status");
    if (lower.includes("resolver.library_search") || lower.includes("library search") || lower.includes("resolver search") || lower.includes("search library"))
    ...
    if (lower.includes("mcp.real_remote_execution") || lower.includes("real remote execution"))
        hits.push("mcp.real_remote_execution");
    if (lower.includes("desktop_bridge.file_scan") || lower.includes("file scan") || lower.includes("file inventory"))
        hits.push("desktop_bridge.file_scan");
    if (lower.includes("desktop_bridge.file_write") || lower.includes("file write"))
        hits.push("desktop_bridge.file_write");
    if (lower.includes("desktop_bridge.file_delete") || lower.includes("file delete") || lower.includes("delete old files"))
        hits.push("desktop_bridge.file_delete");
    if (lower.includes("phasebridge.workflow_plan") || lower.includes("workflow plan"))
        hits.push("phasebridge.workflow_plan");
    if (lower.includes("phasebridge.workflow_execute") || lower.includes("workflow execute"))
        hits.push("phasebridge.workflow_execute");
    return hits.length > 0 ? hits : [value];
}
function inferTaskLevelTools(userTask, knownTools) {
...
function addOrMergeBinding(bindings, binding) {
...
function isDangerousUnknownTool(toolId) {
