...
export function compileRuntimeSpecDryRun(input) {
    ...
    if (selection.candidates.neoblock && selection.active_neoblocks.length > 0)
        selection_events.push(event("NEOBLOCK_SELECTED", "Selected strongest neoblock candidate.", selection.candidates.neoblock));
    if (selection.candidates.molt && selection.active_molt_blocks.length > 0)
        selection_events.push(event("MOLT_BLOCK_SELECTED", "Selected supporting MOLT block candidate.", selection.candidates.molt));
...
function inferRequestedTools(task) {
    ...
    if (lower.includes("langchain"))
        tools.push("langchain_bridge");
    if (lower.includes("agent"))
        tools.push("langchain.agent_mode");
    if (lower.includes("mcp"))
        tools.push("mcp.real_remote_execution");
    return tools;
}
function inferRiskLevel(task, tools) {
    const lower = task.toLowerCase();
    if (tools.some((tool) => tool.includes("mcp")) || lower.includes("approval"))
        return "high";
    if (tools.some((tool) => tool.includes("agent")) || lower.includes("workflow"))
        return "medium";
    return "low";
}
