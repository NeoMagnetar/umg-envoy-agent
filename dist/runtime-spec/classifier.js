export function classifyRuntimeSpecCandidates(input, runtimeArtifacts, supportArtifacts) {
    const selectable = runtimeArtifacts.filter((artifact) => {
        ...
        if (!["manifest", "index", "generated_index"].includes(artifact.source.discovery_method))
            return false;
        if (!["active", "promoted_reference", "curated_first_population_pass", "placeholder_curated_surface_not_populated_yet"].includes(artifact.status))
            return false;
        return true;
    });
