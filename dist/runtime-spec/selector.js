function includesText(artifact, terms) {
...
function boostedScore(artifact, terms, preferredKind) {
    let score = includesText(artifact, terms);
    if (preferredKind && artifact.kind === preferredKind.replace('molt_block', 'molt_block'))
        score += 2;
    const haystack = [artifact.id, artifact.title ?? '', artifact.description ?? ''].join('\n').toLowerCase();
    if (terms.includes('langchain') && haystack.includes('langchain'))
        score += 4;
    if (terms.includes('bridge') && haystack.includes('bridge'))
        score += 3;
    if (terms.includes('sleeve') && artifact.kind === 'sleeve')
        score += 3;
    if (terms.includes('report') && artifact.kind === 'neostack')
        score -= 2;
    if (terms.includes('one-off') && artifact.kind === 'neostack')
        score -= 2;
    return score;
}
export function selectRuntimeArtifacts(input, selectable, support) {
    ...
    let runtime_kind = "assembled_runtime";
    let active_sleeve = null;
    let active_neostacks = [];
    let active_neoblocks = [];
    let active_molt_blocks = [];
    const warnings = [];
