import { canProceedApprovalGatedWrite, createApprovalGatedWritePolicy, createBlockedUnknownToolGate, createProposedActionGate, evaluateApprovalGatedWriteReadiness, } from "./action-gate-types.js";
let passed = 0;
let failed = 0;
...
const dryRunRequiredDecision = evaluateApprovalGatedWriteReadiness(writeGate, capability({ dryRunRequired: true, dryRunSupported: true }), undefined, policy);
assert("dry-run-required write reports dry-run requirement", dryRunRequiredDecision.status === "dry_run_required_before_approval");
const backupRequiredDecision = evaluateApprovalGatedWriteReadiness(writeGate, capability({ backupRequired: true }), { approvalRecord: validApproval }, policy);
assert("backup-required write reports backup requirement", backupRequiredDecision.status === "backup_required");
const rollbackRequiredDecision = evaluateApprovalGatedWriteReadiness(writeGate, capability({ rollbackSupported: true }), { approvalRecord: validApproval }, policy);
...
const readOnlyDecision = evaluateApprovalGatedWriteReadiness(readOnlyGate, capability({
    toolId: "inspect.tool",
...
const lowRiskDecision = evaluateApprovalGatedWriteReadiness(lowRiskGate, capability({
    toolId: "lowrisk.tool",
...
const destructiveDecision = evaluateApprovalGatedWriteReadiness(destructiveGate, capability({
    toolId: "destroy.tool",
...
const transmissionDecision = evaluateApprovalGatedWriteReadiness(transmissionGate, capability({
...
const invalidRuntimeDecision = evaluateApprovalGatedWriteReadiness(invalidRuntimeGate, capability({ toolId: "unsafe-write.tool", toolName: "Unsafe Write Tool" }), undefined, policy);
...
const invalidTraceDecision = evaluateApprovalGatedWriteReadiness(invalidTraceGate, capability({ toolId: "unsafe-trace-write.tool", toolName: "Unsafe Trace Write Tool" }), undefined, policy);
