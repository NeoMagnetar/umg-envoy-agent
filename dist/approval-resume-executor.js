...
const resumeExecutionStore = new Map();
const PHASE42_ALLOWLIST = new Set(["umg_envoy_matrix_status"]);
function trace(record, event_type, message, data = {}) {
    return {
        event_type,
...
export async function executeApprovalResume(record, binding) {
    const traceEvents = [trace(record, "APPROVAL_RESUME_EXECUTION_REQUESTED", "Approval resume execution requested.", { approval_id: record.approval_id })];
