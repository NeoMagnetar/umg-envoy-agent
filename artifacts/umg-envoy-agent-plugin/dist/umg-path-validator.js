...
function hasBasicIdShape(value) {
...
function collectMoltStates(doc) {
    const states = new Map();
    for (const stack of doc.stacks) {
        for (const block of stack.blocks) {
            for (const molt of block.molts) {
                const current = states.get(molt.id) ?? new Set();
...
function collectNodeKinds(doc) {
    const map = new Map();
    if (doc.sleeveId)
        map.set(doc.sleeveId, "sleeve");
    for (const trigger of doc.triggers)
    ...
    for (const stack of doc.stacks) {
        ...
        for (const block of stack.blocks) {
            ...
            for (const molt of block.molts)
                map.set(molt.id, "molt");
        }
    }
    return map;
}
function validateIdFields(doc, issues) {
    if (!doc.use)
...
function validateMoltStateConflicts(doc, issues) {
...
function validateRelationships(doc, issues, ids, nodeKinds) {
...
function validateCrossDeclarationConflicts(doc, issues) {
...
function validateWinners(doc, issues, ids) {
...
function validateCompilerStages(doc, issues) {
...
export function isCanonicalRelationshipKind(kind) {
    return canonicalRelationshipKinds().has(kind);
}
export function plannerValidatorStage() {
    return "stage-2-hardened";
}
export function plannerValidatorCapabilities() {
    return [
