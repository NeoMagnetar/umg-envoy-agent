...
function toAbsoluteFrom(baseDir, maybeRelative) {
...
export function readSleeveCatalog(paths) {
    return readJsonFile(paths.sleeveCatalogPath);
}
export function listSleeves(paths) {
...
export function resolveSleeveSourcePath(paths, sleeveId) {
    ...
    if (!match.resolvedSourcePath || !fs.existsSync(match.resolvedSourcePath)) {
        throw new Error(`Sleeve '${sleeveId}' does not currently resolve to a readable source file. Catalog path: ${match.source_path ?? "(missing)"}`);
    }
    return match.resolvedSourcePath;
}
export function readActiveSleeve(paths) {
    return readJsonFile(paths.activeSleevePath);
}
export function readActiveStack(paths) {
    return readJsonFile(paths.activeStackPath);
}
export function compareSleeves(paths, leftSleeveId, rightSleeveId) {
    const leftPath = resolveSleeveSourcePath(paths, leftSleeveId);
    const rightPath = resolveSleeveSourcePath(paths, rightSleeveId);
    const left = readJsonFile(leftPath);
    const right = readJsonFile(rightPath);
    ...
    return {
        ...
        differences: {
            ...
            stacksOnlyInLeft: leftStacks.filter((item) => !rightStacks.includes(item)),
            stacksOnlyInRight: rightStacks.filter((item) => !leftStacks.includes(item)),
            stackCountLeft: leftStacks.length,
            stackCountRight: rightStacks.length
        }
    };
}
function summarizeCompiledRuntime(compiled) {
...
export function summarizeActiveRuntime(paths) {
    const sleeve = readActiveSleeve(paths);
    const stack = readActiveStack(paths);
    const runtimeSpec = stack?.runtimeSpec ?? null;
...
export function validateCompiledRuntime(compiled) {
    const errors = [];
    const warnings = [];
...
function buildTraceSummary(compiled, sourcePath, sleeveId) {
    const runtimeSummary = summarizeCompiledRuntime(compiled);
    return {
        generatedBy: "openclaw-umg-envoy-agent",
...
export async function compileSleeveById(paths, sleeveId, options = {}) {
    ...
    const parsedRuntime = readJsonFile(outputPath);
    if (parsedRuntime?.hasErrors && !parsedRuntime?.runtime) {
        ...
        const errorMessages = traceEvents
            .filter((event) => event?.severity === "error" || String(event?.code ?? "").startsWith("ERR_"))
            .map((event) => event?.message)
            .filter((message) => typeof message === "string" && message.trim().length > 0);
        throw new Error(`Compiler reported an invalid sleeve payload for '${sleeveId}': ${errorMessages.join(" | ") || "unknown compiler validation failure"}`);
    }
...
export function previewPromotion(paths, compiledOutputPath, sleeveId, promotionLabel) {
    ...
    const compiled = readJsonFile(compiledOutputPath);
...
function createPromotionBackup(paths, sleeveId, promotionLabel, compiledOutputPath) {
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    ...
    const previousActiveSleeve = fs.existsSync(paths.activeSleevePath)
        ? readJsonFile(paths.activeSleevePath)
        : null;
    const previousActiveStack = fs.existsSync(paths.activeStackPath)
        ? readJsonFile(paths.activeStackPath)
        : null;
...
export function promoteCompiledRuntime(paths, config, compiledOutputPath, sleeveId, promotionLabel) {
    if (!config.allowRuntimeWrites) {
        throw new Error("Runtime writes are disabled. Enable allowRuntimeWrites in plugin config before using promote operations.");
    }
    if (!fs.existsSync(compiledOutputPath)) {
        throw new Error(`Compiled runtime output not found: ${compiledOutputPath}`);
    }
