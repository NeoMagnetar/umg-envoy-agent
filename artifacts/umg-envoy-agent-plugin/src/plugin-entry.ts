...
function pickAllowRuntimeWritesOverride(value: unknown): Partial<PluginConfig> | undefined {
...
function registerExperimentalReplyTriggerHooks(api: any, config?: PluginConfig) {
  const debugDirectReplyBehavior = Boolean(config?.debugDirectReplyBehavior);
  if (typeof api.registerHook !== "function") {
    return;
  }

  const exactResponseBySession = new Map<string, string>();

  api.registerHook(
    "before_prompt_build",
    async (event: { sessionId?: unknown; prompt?: string; messages?: Array<{ role?: unknown; content?: unknown }> }) => {
  ...
  api.registerHook(
    "message_sending",
    async (event: { content?: unknown }, ctx: { sessionKey?: unknown }) => {
    ...
    api.registerTool(
      {
        ...
        description:
          "Inspect detected activation candidates, canonical runtime activation payload, and derived compiler-v0 triggerState for a message.",
        ...
        async execute(_id: string, params: { message: string; sleeveId?: string }) {
          const paths = resolvePaths(config ?? {});
          const trace = buildActivationTraceView({
            paths,
    ...
    api.registerTool(
      {
        ...
        description:
          "Run a small end-to-end smoke test from message input through runtime activation, compiler-v0 triggerState adaptation, and compiler output/trace.",
        parameters: Type.Object(
          {
